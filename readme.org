...
This fork replaces the Mouse layer with an [[https://github.com/nikitabobko/AeroSpace][AeroSpace]] window-management layer, defined in [[miryoku/custom_config.h]].  The layer keeps the ~MOUSE~ id, so the layer lock keys on the other layers are unchanged; only the label is changed to "Window".  Mouse movement, scrolling, and mouse buttons are removed.

Hold Tab (left thumb) to activate.  Every key sends ~ctrl-alt~ (macOS Control+Option) plus the key shown, matching the AeroSpace bindings in [[https://github.com/ae-bii/nix-config/blob/main/modules/darwin/aerospace.nix][nix-config]].  Right hand, inner column to pinky:
...
| Bottom | toggle floating | shrink     | grow       | toggle orientation | tiles/accordion |
| Thumbs |                 | balance sizes |         |                    |                 |

The left hand keeps the Miryoku home-row mods.  Holding Shift there while on the layer sends the shifted AeroSpace command:

| Key             | With Shift                |
|-----------------+---------------------------|
| focus           | move window               |
| workspace       | move window to workspace  |
| shrink / grow   | resize opposite axis      |
| tiles/accordion | flatten workspace tree    |
...
AeroSpace window management; every key sends ctrl-alt plus the key.  See [[#window-layer][Window Layer]].

| Boot | to Tap | to Extra | to Base   |     | ws 1       | ws 2    | ws 3    | ws 4    | ws 5      |
| GUI  | Alt    | Ctrl     | Shift     |     | fullscreen | focus ← | focus ↓ | focus ↑ | focus →   |
|      | AltGr  | to Sym   | to Window |     | float      | shrink  | grow    | orient  | tiles/acc |
|------+--------+----------+-----------+-----+------------+---------+---------+---------+-----------|
|      |        |          |           |     |            | balance |         |         |           |
